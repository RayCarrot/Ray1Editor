...
namespace Ray1Editor.Updater
{
    ...
    public class UpdaterViewModel : BaseViewModel
    {
        ...
        public async Task UpdateAsync(CancellationToken cancellationToken)
        {
            ...
            AddLog($"App path retrieved as {App.R1EFilePath}");

            // Get the file URL
            ServerFileURL = App.UpdateURL;
            AddLog($"Server file URL detected as {ServerFileURL}");

            AddLog($"Temp download path generated as {App.ServerTempPath}");
            ...
            AddLog($"Temp local backup path generated as {App.LocalTempPath}");
            ...
            try
            {
                ...
                File.Move(App.R1EFilePath, App.LocalTempPath);
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (File.Exists(App.R1EFilePath))
                    File.Delete(App.R1EFilePath);

                // Move downloaded update to old program's path
                File.Move(App.ServerTempPath, App.R1EFilePath);
            }
            catch (Exception ex)
            {
                try
                {
                    ...
                    File.Move(App.LocalTempPath, App.R1EFilePath);
                }
                catch (Exception ex2)
                {
            ...
            Process.Start(App.R1EFilePath)?.Dispose();
