...
namespace RayCarrot.Ray1Editor
{
    ...
    public static class Primitives2D
    {
        ...
        private static Dictionary<string, List<Vector2>> CircleCache { get; } = new Dictionary<string, List<Vector2>>();

        private static Texture2D _pixel;
        private static Texture2D GetPixel(GraphicsResource spriteBatch) => _pixel == null || _pixel.IsDisposed ? (_pixel = CreatePixel(spriteBatch)) : _pixel;

        #endregion

        #region Private Methods

        private static Texture2D CreatePixel(GraphicsResource spriteBatch)
        {
            var p = new Texture2D(spriteBatch.GraphicsDevice, 1, 1, false, SurfaceFormat.Color);
            p.SetData(new[]
            {
                Color.White
            });
        ...
        {
            spriteBatch.Draw(GetPixel(spriteBatch), rect, null, color, angle, Vector2.Zero, SpriteEffects.None, 0);
        }
        ...
        /// <param name="angle">The angle in radians to draw the rectangle at</param>
        /// <param name="color">The color to draw the rectangle in</param>
        public static void DrawFilledRectangle(this SpriteBatch spriteBatch, Vector2 location, Vector2 size, Color color, float angle)
        {
        ...
        public static void DrawFilledRectangle(this SpriteBatch spriteBatch, float x, float y, float w, float h, Color color, float angle)
        {
